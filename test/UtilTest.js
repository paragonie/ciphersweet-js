...
describe('Util', function () {
    it('Util.andMask()', function () {
        ...
        for (let i = 0; i < testCases.length; i++) {
            ...
            expect(outputRight).to.be.equal(
                Util.andMask(
                    Buffer.from(input, 'hex'),
                    size,
                    true
    ...
    it('Util.hmac()', function () {
        let out = Util.hmac("sha256", "Paragon Initiative Enterprises", "Happy Pie Day!");
        expect(
    ...
    it('Util.HKDF() -- (RFC 5869) test vectors', function () {
        ...
        let out = Util.HKDF(ikm, salt, info, 42, 'sha256');
